userInfo = {
  ...
  function pubg.getRealY(options, y)
    local realY = y * pubg[pubg.scope_current]
    if IsModifierPressed("lctrl") then
      -- OutputLogMessage("P:" .. realY .. "\n")
      -- realY = realY * options.ctrlmodeRatio
      realY = math.ceil(realY * options.ctrlmodeRatio)
    -- OutputLogMessage(options.ctrlmodeRatio .. "\n")
    -- OutputLogMessage("O:" .. realY .. "\n")
    end
    return realY
  end
